...
local function validate(embed)
   ...
   if json.fields then
      for i = 1, #json.fields do
         assert(
            #json.fields[i].name <= 256,
            'Field names must be less than or equal to 256 characters'
         )

         assert(
            #json.fields[i].value <= 1024,
